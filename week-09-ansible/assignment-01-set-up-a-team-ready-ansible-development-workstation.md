...
![output ](screenshots/A1Screenshot9.5.png)
...
One feature that makes my workstation setup team-friendly is the use of a project-level ansible.cfg together with a Python virtual environment and requirements.txt. This makes the Ansible tools and configuration consistent and reproducible for other team members instead of depending on their system-wide Python or Ansible installation.

---

**2. What is one pitfall you avoided while completing the setup?**

One pitfall I avoided was exposing sensitive files such as SSH private keys and the Python virtual environment to Git. I added these items to .gitignore so they remain local to the workstation. I also encountered a project-directory permission issue in WSL where Ansible ignored the local ansible.cfg because the directory was world-writable. Fixing the permissions allowed Ansible to recognize the project configuration correctly.

---

**3. Why should Ansible be installed inside a Python virtual environment?**

Ansible should be installed inside a Python virtual environment because it isolates the project dependencies from the system Python installation. This prevents version conflicts with other Python projects and makes it easier to reproduce the same Ansible environment on another workstation. It also allows Ansible and tools such as ansible-lint, yamllint, and pre-commit to be managed together.

---

**4. Why must SSH private keys and `.venv/` remain outside version control?**

SSH private keys must remain outside version control because anyone who obtains the private key may be able to authenticate to systems that trust it. The .venv/ directory should also remain outside version control because it contains locally installed Python packages and environment-specific files that can be recreated from requirements.txt. Keeping both out of Git reduces security risks and keeps the repository clean and portable.
